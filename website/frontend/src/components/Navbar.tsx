...
export default function Navbar() {
  ...
  return (
    <div className="flex flex-col w-full z-50 sticky top-0">
      ...
      <div className="bg-brand-accent text-white h-8 flex items-center text-[10px] md:text-xs font-bold hidden md:flex">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 flex justify-between items-center gap-8">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-2 border-l border-white/30 pl-8">
      ...
      <header className="bg-white/95 backdrop-blur-xl shadow-[0_8px_32px_-12px_rgba(0,0,0,0.08)] border-b border-brand-accent/5 transition-all duration-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-[64px] lg:h-[76px]">
            ...
            {/* Logo area */}
            <div className="flex-shrink-0 flex items-center pr-2 md:pr-8">
              <Link to="/" className="flex items-center gap-3 transition-transform hover:scale-[1.02]">
                ...
                <div className="flex flex-col hidden sm:flex">
                  <span className="font-serif text-[15px] sm:text-[19px] font-bold text-brand-primary tracking-tight leading-none">CITY EDUCATIONAL</span>
            ...
            <nav className="hidden md:flex space-x-1 lg:space-x-4 flex-grow justify-center">
              {navLinks.map((link) => (
                ...
                >
                  <Link 
                    to={link.path} 
                    className={`relative px-3 py-4 text-xs xl:text-sm font-bold tracking-wider transition-colors duration-300 flex items-center gap-1 whitespace-nowrap ${
            ...
            <div className="hidden md:flex items-center pl-8">
              <Link 
                to="/admissions" 
                className="glow-on-hover inline-flex items-center justify-center px-6 py-2.5 text-xs xl:text-sm font-bold rounded-full text-white bg-brand-accent transition-all shadow-lg shadow-brand-accent/20 tracking-wide uppercase group/cta overflow-hidden whitespace-nowrap"
              >
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              ...
              className="md:hidden bg-white/95 backdrop-blur-xl border-t border-brand-accent/10 absolute left-0 w-full shadow-2xl z-50 overflow-hidden"
            >
